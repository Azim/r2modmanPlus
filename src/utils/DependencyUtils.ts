...
export async function getFullDependencyList(combos: ThunderstoreCombo[], game: Game, profile: ImmutableProfile) {
    for (const combo of combos) {
        ...
        combos = isModpack ? combos :
            combos.filter((dep) => {
                return !((modList as ManifestV2[]).some(installed => installed.getName() === dep.getMod().getFullName()));
            });
    }
